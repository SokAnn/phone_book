        ...
        if self.NamelineEdit.text() and self.TelephonelineEdit.text() and self.dateEdit.text():
            ...
            if check == 1:
                if datetime.datetime.strptime(formating_date(self.dateEdit.text()), "%Y-%m-%d") > datetime.datetime.now():
                    ...
                    m.setText("Дата рождения из будущего!")
                    m.setIcon(QMessageBox.Critical)  # set icon
                    m.setStandardButtons(QMessageBox.Ok)
                    m.exec_()
                else:
                    self.cur.execute("UPDATE db_users.users SET Telephone = ?, BDate = ? WHERE Name = ?",
                                     (self.TelephonelineEdit.text(), formating_date(self.dateEdit.text()),
                ...
                m.setText("Нельзя изменить контакт, которого нет!\n"
                          "Попробуйте добавить такой контакт или изменяйте существующий!")
                m.setIcon(QMessageBox.Critical)  # set icon
                m.setStandardButtons(QMessageBox.Ok)
                m.exec_()
        else:
